...
function updateSaveButtonState() {
  ...
  const hasChanges = window._configEdits && Object.keys(window._configEdits).length > 0;
...
async function saveConfig() {
  ...
  try {
    ...
    if (window.ahk?.global?.UpdateConfigBulk) {
      const result = await window.ahk.global.UpdateConfigBulk(JSON.stringify(changes));
      showToast(result ? 'Configuration saved!' : 'Failed to save configuration');
    } else {
      ...
      showToast(allSuccess ? 'Configuration saved!' : 'Some settings failed to save');
    }
